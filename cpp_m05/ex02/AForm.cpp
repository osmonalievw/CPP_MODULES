...
AForm::AForm(std::string Name, int rankSign, int rankExec)
    ...
    if (_gradeToExecute < 1 || _gradeToSign < 1) throw AForm::GradeTooHighException();
    if (_gradeToExecute > 150 || _gradeToSign > 150) throw AForm::GradeTooLowException();
...
void AForm::beSigned(Bureaucrat& v) {
    if (v.getGrade() > this->getSignGrade()) throw AForm::GradeTooLowException();
    _isSigned = true;
}
...
void AForm::checkExecutionRequirements(Bureaucrat const& executor) const {
    ...
    if (executor.getGrade() > _gradeToExecute) throw AForm::GradeTooLowException();
}
